...
class Admin::WeightsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @weight.save
        format.html { redirect_to [:admin,@weight], notice: 'Weight was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @weight.update_attributes(params[:weight])
        format.html { redirect_to [:admin,@weight], notice: 'Weight was successfully updated.' }
        format.json { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_weights_url }
      format.json { head :ok }
    end
  end
end
